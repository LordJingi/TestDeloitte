		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <Batch>");
				},
				iSortTheListOnUnitNumber : function () {
					return this.iChooseASorter("sortButton", "Sort By <QuantityInBaseUnit>");
				},

				iFilterTheListOnUnitNumber : function () {
					return this.iMakeASelection("filterButton", "<QuantityInBaseUnit>", "<100 <CompanyCodeCurrency>");
				},

				iGroupTheList : function () {
					return this.iChooseASorter("groupButton", "<QuantityInBaseUnit> Group");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "A_MaterialDocumentItem",
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("MaterialDocument") === oObject.MaterialDocument;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].MaterialDocument;
									} else {
										sCurrentId = aItemsNotInTheList[0].MaterialDocument;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("A_MaterialDocumentItem", {
										MaterialDocument : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("MaterialDocument"),
						title: oBindingContext.getProperty("Batch")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<A_MaterialDocumentItem> (0)"}),
						success: function () {
				...
				theListShouldBeFilteredOnUnitNumber : function () {
					return this.theListShouldBeFilteredOnFieldUsingComparator("QuantityInBaseUnit", 100);
				},


				theListShouldBeSortedAscendingOnUnitNumber : function () {
					return this.theListShouldBeSortedAscendingOnField("QuantityInBaseUnit");
				},

				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("Batch");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all A_MaterialDocumentItem to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<A_MaterialDocumentItem> (" + iExpectedLength + ")"}),
								success : function () {
